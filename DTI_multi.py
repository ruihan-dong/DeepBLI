...
class DBTA:
	...
	def test_(self, data_generator, model, repurposing_mode = False, test = False):
		...
		return roc_auc_score(y_label, y_pred), average_precision_score(y_label, y_pred), accuracy_score(y_label, outputs), f1_score(y_label, outputs), log_loss(y_label, outputs), y_pred
	...
	def train(self, train1, val1, test1, train2, val2 = None, test2 = None, verbose = True):
		...
		training_generator = data.DataLoader(data_process_loader_2(train1.index.values, train1.Label.values, train1, 
																train2.index.values, train2, **self.config), **params)

		if val1 is not None:
			validation_generator = data.DataLoader(data_process_loader_2(val1.index.values, val1.Label.values, val1,
																		val2.index.values, val2, **self.config), **params)

		if test1 is not None:
			info = data_process_loader_2(test1.index.values, test1.Label.values, test1, test2.index.values, test2,**self.config)
			...
			testing_generator = data.DataLoader(data_process_loader_2(test1.index.values, test1.Label.values, test1,
		...
		valid_metric_header.extend(["AUROC", "AUPRC", "Acc", "F1"])
		...
		for epo in range(train_epoch):
			...
			if val1 is not None:
				...
				with torch.set_grad_enabled(False):
					## binary: ROC-AUC, PR-AUC, ACC, F1, cross-entropy loss
					auc, auprc, acc, f1, loss, logits = self.test_(validation_generator, self.model)
					lst = ["epoch " + str(epo)] + list(map(float2str,[auc, auprc, acc, f1]))
		...
		if test1 is not None:
			...
			# with torch.no_grad():
				auc, auprc, acc, f1, loss, logits = self.test_(testing_generator, model_max, test = True)
			test_table = PrettyTable(["AUROC", "AUPRC", "ACC", "F1"])
			test_table.add_row(list(map(float2str, [auc, auprc, acc, f1])))
			# epo = 0
			if verbose:
				print('Validation at Epoch '+ str(epo + 1) + ' , AUROC: ' + str(auc)[:7] + \
					' , AUPRC: ' + str(auprc)[:7] + ' , Acc: '+str(acc)[:7] +' , F1: '+str(f1)[:7] + ' , Cross-entropy Loss: ' + \
	...
	def predict(self, df_data_1, df_data_2):
		...
		'''
		print('predicting...')
		info = data_process_loader_2(df_data_1.index.values, df_data_1.Label.values, df_data_1, 
